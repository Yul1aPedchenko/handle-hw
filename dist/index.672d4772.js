...
refs.modal.modalForm.addEventListener("submit", (e)=>{
    ...
    const updatedStudent = {
        ...
        skills: document.getElementById("edit-skills").value.split(",").map((skill)=>skill.trim()).filter((skill)=>skill),
    ...
    const markUp = students.map((student)=>{
        return `<tr>
            ...
            <td>${student.isEnrolled}</td>
             <td> 
            <button id="update-student-btn" data-id="${student.id}">\u{420}\u{435}\u{434}\u{430}\u{433}\u{443}\u{432}\u{430}\u{442}\u{438}</button>
            <button id="delete-student-btn" data-id="${student.id}">\u{412}\u{438}\u{434}\u{430}\u{43B}\u{438}\u{442}\u{438}</button></td>
        </tr>`;
    });
